    ...
    toast.custom((t) => (
      <div className={`${t.visible ? 'animate-in fade-in zoom-in' : 'animate-out fade-out zoom-out'} max-w-md w-full bg-white shadow-2xl rounded-[2rem] flex border border-slate-100 overflow-hidden`}>
        ...
        <div className="flex flex-col border-l border-slate-100 w-32">
          ...
          >Confirm</button>
          <button onClick={() => toast.dismiss(t.id)} className="flex-1 text-[10px] font-black uppercase text-slate-400">Cancel</button>
        </div>
      </div>
    ));
  };
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-5xl font-black tracking-tighter text-slate-900 leading-none italic uppercase mb-2">
              Schedule<span className="text-cyan-500 italic">.Management</span>
            </h1>
          ...
          <div className="flex items-center gap-4 bg-white p-2 rounded-[2.5rem] border border-slate-200 shadow-xl">
            <div className="bg-slate-900 text-white px-8 py-4 rounded-[2rem] flex flex-col items-center min-w-[140px]">
            ...
            <div className="flex items-center gap-8 px-6 border-l border-slate-100">
              <div className="hidden md:block">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Live Load</p>
                <div className="flex items-center gap-3">
                  <div className="h-1.5 w-24 bg-slate-100 rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${(stats.bookedToday / stats.totalToday) * 100 || 0}%` }} className="h-full bg-indigo-500" />
                  </div>
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-emerald-500 uppercase tracking-tighter flex items-center gap-1.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /> Free Slots
                </span>
        ...
        <motion.section className="bg-slate-900 rounded-[3rem] p-10 mb-10 text-white shadow-2xl relative overflow-hidden">
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
             <div className="lg:col-span-4">
                ...
                <select value={selectedField} onChange={(e) => setSelectedField(e.target.value)} className="w-full bg-slate-800 border-none p-5 rounded-2xl font-bold text-sm text-white focus:ring-2 focus:ring-indigo-500 transition-all outline-none">
             ...
             <div className="lg:col-span-3">
                ...
                <input type="date" value={genDate} onChange={(e) => setGenDate(e.target.value)} className="w-full bg-slate-800 border-none p-5 rounded-2xl font-bold text-sm text-white outline-none" />
             </div>
             <div className="lg:col-span-3">
             ...
             <div className="lg:col-span-2">
                <button onClick={() => genMutation.mutate({ field_id: selectedField, date: genDate, start_hour: startH, end_hour: endH })} disabled={genMutation.isPending || !selectedField} className="w-full py-5 bg-indigo-600 hover:bg-indigo-500 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-3">
        ...
        <div className="space-y-6">
          <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-4 w-full lg:w-auto">
              ...
              <AnimatePresence>
                {selectedIds.length > 0 && (
                  <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} onClick={handleBulkDelete} className="flex items-center gap-2 bg-rose-500 text-white px-6 py-4 rounded-full font-black text-[10px] uppercase tracking-widest">
            ...
            <div className="relative w-full lg:max-w-md group">
              ...
              <input type="text" placeholder="Search Arena..." className="w-full pl-16 pr-8 py-5 bg-white rounded-full border border-slate-200 font-bold text-xs outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all shadow-sm" onChange={(e) => setSearchText(e.target.value)} />
            </div>
          </div>

          {/* TABLE AREA - FIXED STUTTERING */}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100 uppercase text-[10px] font-black text-slate-400">
                    <th className="px-10 py-7 w-12 text-center">
                       <input type="checkbox" onChange={(e) => setSelectedIds(e.target.checked ? filteredSchedules.filter(s => s.status === 'available').map(s => s.id) : [])} className="w-4 h-4 rounded-md border-slate-300 text-indigo-600" />
                    </th>
                    <th className="px-6 py-7 tracking-widest">Time Slot</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {/* Kita lepas AnimatePresence dari per-row jika data sangat banyak untuk performa maksimal */}
                  {sLoading ? (
                    <tr><td colSpan="5" className="p-32 text-center"><Loader2 className="animate-spin inline-block text-indigo-500" size={40} /></td></tr>
                  ) : filteredSchedules.map((s) => (
                      <motion.tr 
                        key={s.id} 
                        // Hilangkan prop layout untuk mencegah efek "jatuh"
                        initial={{ opacity: 0 }} 
                        animate={{ opacity: 1 }} 
                        className={`group hover:bg-slate-50/80 transition-all ${selectedIds.includes(s.id) ? 'bg-indigo-50/30' : ''}`}
                      >
                        <td className="px-10 py-6 text-center">
                          <input type="checkbox" checked={selectedIds.includes(s.id)} disabled={s.status === 'booked'} onChange={() => toggleSelect(s.id)} className="w-4 h-4 rounded-md border-slate-300 text-indigo-600 disabled:opacity-20" />
                        </td>
                        <td className="px-6 py-6">
                           <div className="flex items-center gap-4">
                             <div className={`p-3 rounded-xl transition-all ${selectedIds.includes(s.id) ? 'bg-indigo-500 text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-white group-hover:text-indigo-600'}`}>
                                <Calendar size={18} />
                             </div>
                             <div>
                               <div className="text-xs font-black text-slate-900 uppercase italic">{new Date(s.start_time).toLocaleDateString("id-ID", { weekday: 'long', day: 'numeric', month: 'short' })}</div>
